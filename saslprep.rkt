...
(define (map-to-space? c)   (char-in-set? c map-to-space-v))
(define (map-to-nothing? c) (char-in-set? c map-to-nothing-v))
...
(define (prohibited-char? c) (char-in-set? c prohibited-v))
(define (unassigned-char? c) (char-in-set? c unassigned-v))
...
(define (do-check-bidi s orig)
  ...
  (cond [(zero? (string-length s)) (void)]
        [(for/or ([c (in-string s)]) (RandAL-char? c))
         (for ([c (in-string s)])
           (when (L-char? (char->integer c))
             (bad "string contains both RandALCat and LCat characters")))
         (unless (and (RandAL-char? (string-ref s 0))
                      (RandAL-char? (string-ref s (sub1 (string-length s)))))
           (bad "string containing RandALCat characters must start and end with RandALCat"))]
        [else (void)]))

(define (RandAL-char? c) (char-in-set? c RandALCat-v))
(define (L-char? c)      (char-in-set? c LCat-v))

;; ----------------------------------------
;; Character range tables

(define map-to-space-v
  '#(#x00A0 #x00A0
     #x1680 #x1680
     #x2000 #x200B
     #x202F #x202F
